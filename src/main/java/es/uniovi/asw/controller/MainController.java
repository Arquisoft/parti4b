...
import es.uniovi.asw.participationSystem.parser.impl.RList;
import es.uniovi.asw.participationSystem.parser.writer.FactoryLetter;
import es.uniovi.asw.participationSystem.parser.writer.Letter;
import es.uniovi.asw.participationSystem.parser.writer.PDFLetter;
import es.uniovi.asw.participationSystem.parser.writer.TXTLetter;
import es.uniovi.asw.participationSystem.parser.writer.WordLetter;
...
//		kafkaProducer.send("logout", "El usuario cerro sesion correctamente" + " o es la primera vez que entrea");
	...
	public ModelAndView login(@RequestParam("dni") String dni, @RequestParam("password") String password, HttpSession session) {
		...
		if (usuario != null) {
			if (usuario.isAdmin()) {
				kafkaProducer.send(Topics.LOGGIN, "" + usuario.getId());
		...
		if (user != null) {
			...
			if (user.isAdmin()) {
				...
				for (int i = 0; i < commentaries.size(); i++) {
					...
					imprime.setNombre(factory.getServicesFactory().getCitizenService().findById(commentaries.get(i).getCitizen().getId()).getNombre());
	...
	public ModelAndView crearPropuesta(@RequestParam("nombre") String nombre, @RequestParam("contenido") String contenido, HttpSession session) {
		Citizen user = (Citizen) session.getAttribute("user");
		if (user != null) {
			...
			List<Proposal> proposals = factory.getServicesFactory().getProposalService().findByStatus(EstadosPropuesta.EnTramite);
			kafkaProducer.send(Topics.NEW_PROPOSAL, "" + propuesta.getId());
			return new ModelAndView("usuario").addObject("proposals", proposals);
		} else {
	...
	public ModelAndView votarPositivo(@RequestParam("idPropuesta") String idPropuesta, HttpSession session) {
		...
		if (user != null) {
			...
			if (!propuesta.positiveVote(user)) {
				kafkaProducer.send(Topics.ILEGAL_VOTE, "" + propuesta.getId());
				return new ModelAndView("usuario").addObject("proposals", proposals).addObject("hiddenFalse", true).addObject("mensaje", "El usuario ya ha votado esta propuesta");
			} else {
...
//			kafkaProducer.send("user", "Voto realizado");
			return new ModelAndView("usuario").addObject("proposals", proposals).addObject("hiddenTrue", true).addObject("mensaje", "Voto realizado correctamente");
		} else {
...
//					kafkaProducer.send("admin", "Propuestas en trámite");
					return new ModelAndView("enTramiteAdmin").addObject("proposals", proposals).addObject("hidden", false).addObject("id", idPropuesta);
				} else {
//					kafkaProducer.send("admin", "Propuestas en trámite");
...
//			kafkaProducer.send("user", "Error en propuestas en trámite");
	...
	public ModelAndView propuestasRechazadas(HttpSession session) {
		Citizen user = (Citizen) session.getAttribute("user");
		if (user != null) {
			List<Proposal> proposals = factory.getServicesFactory().getProposalService().findByStatus(EstadosPropuesta.Rechazada);
			if (user.isAdmin()) {
				if (proposals != null) {
...
//			kafkaProducer.send("admin", "Error en propuestas rechazadas");
	...
	public ModelAndView propuestasAceptadas(HttpSession session) {
		Citizen user = (Citizen) session.getAttribute("user");
		if (user != null) {
			List<Proposal> proposals = factory.getServicesFactory().getProposalService().findByStatus(EstadosPropuesta.Aceptada);
			if (user.isAdmin()) {
				if (proposals != null) {
//					kafkaProducer.send("admin", "Propuestas aceptadas");
					return new ModelAndView("aceptadasAdmin").addObject("proposals", proposals).addObject("hidden", false);
				} else {
//					kafkaProducer.send("admin", "Propuestas aceptadas");
...
//			kafkaProducer.send("admin", "Error al rechazar una propuesta");
	...
	public ModelAndView modificarMinVotes(@RequestParam("minVotes") int minVotes, @RequestParam("idPropuesta") String idPropuesta, HttpSession session) {
		Citizen user = (Citizen) session.getAttribute("user");
		if (user != null) {
			Proposal propuesta = factory.getServicesFactory().getProposalService().findById(Long.parseLong(idPropuesta));
...
//			kafkaProducer.send("admin", "Accediendo a loadUsers");
			...
			try {
				if (censo != null) {
					...
					List<Citizen> citizens = new RList().readCitizens(censo);
...
//			kafkaProducer.send("admin", "Acceso denegado");
	...
	public ModelAndView dashboard(HttpSession session) {
		...
		if (user != null) {
			if (user.isAdmin())
				return new ModelAndView("dashboard").addObject("proposals", EventControllerImpl.votedProposals)
						.addObject("hiddendash", true);
		}
